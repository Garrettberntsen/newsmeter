class PageViewsController < ApplicationController
	...
	def send_page
		...
		# Save the time at which the user left the page
		...
		begin
			...
			if Page.select(:canonical_url).pluck(:canonical_url).include? params[:canonicalurl]
				new_page_view = PageView.new :user_id => (session[:user_id].nil? ? params[:user].to_i : session[:user_id]), :actual_url => (params[:url] == "" ? nil : params[:url]), :page_id => (Page.find_by canonical_url: params[:canonicalurl]).id, :ip_address => request.remote_ip
				new_page_view.save
			else
